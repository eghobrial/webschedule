...
* ChangeEvent on the General test version
* @version
...
*
* March 2013 added the ability to modify an event within 2 hours after the end date
*--------------------------------------------------------------*/
...
else if  (check_date_change(s_eventID)==1) return showPage("You can not modify part of an event 2 hours after end time");
...
//temp_hold_change change_info = new temp_hold_change(projID, descrip,starthi,startmi, endhi,endmi,day,month, year, dow);
    ...
    {
        String s_eventID = this.getS_eventID();
        int starthi=0, startmi=0, endhi=0, endmi=0, schh=0, schm=0,schday=0,schmonth=0,schyear=0;
   	String projID=null;
   	int check_user_flag = 0;
	double starttime;
   	double endtime;
		...
		try {
	    	S_event s_event = S_eventFactory.findS_eventByID(s_eventID);
            	...
            	endhi = s_event.getEndh();
            	endmi = s_event.getEndm();
		schday = s_event.getTodayday();
		schmonth = s_event.getTodaymonth();
		schyear = s_event.getTodayyear();
		schh = s_event.getTodayh();
		schm = s_event.getTodaym();
	...
	//calculation for the time when the event was scheduled
	Calendar schinfo = Calendar.getInstance();
    	schinfo.set(schyear, schmonth, schday, schh, schm);
    	Date schinfodate =   schinfo.getTime();
    	long schinfoms = schinfodate.getTime();
	long difference =  cancelinfoms - schinfoms;
	...
	if ((starttime < 20) && (difference > 9000000))
	   {
	   System.out.println("Is before 8pm and has been booked for longer that two and half hours add to r_event");
		saveR_event(projID,starthi,endhi,startmi,endmi,s_eventID);
	 }	
		// added to subtract 1 hour from the Pilot project hour.
		//subtract_pilot_proj_hours(projID,starthi,endhi,startmi,endmi,s_eventID);
		//save this event as canceled
		saveC_event(s_eventID,theC_event);
		//send an email that a slot has been deleted
...
//	String password = this.getComms().request.getParameter(PASSWORD_NAME);
	...
	// deleting an event right before it starts
 ...
 public int check_date_change(String s_eventID)
	...
	if (numofhours < 0)
	{
System.out.println("Check date is OK off ChangeEvent module +++"+Long.toString(numofhours));
		return 1;
		}
	else 	
        	return 0;
      ...
      {
       String s_eventID = this.getS_eventID();
        System.out.println("Handle Change  visited  ");
       if  (check_user(s_eventID)==1) return showPage("You can not delete part of another user event");
        else if  (check_date(s_eventID)==1) return showPage("You can not delete a past event");
/*        else if  (check_date_change(s_eventID)==1) return showPage("You can not modify part
of a past event");*/
        else throw new ClientPageRedirectException(CHANGE_PAGE);
      }
 ...
 theProject.setExp(theProject.isExp());
  ...
  /*Update pilot project hours in the canelation case*/

  protected void subtract_pilot_proj_hours(String projID,int starthi,int endhi,int startmi,int endmi, String s_eventID)
   throws HttpPresentationException
   {
   	double starttime;
   	double endtime;
...
//    	totalhours = pilotProject.getTotalhours()-(eventhours);
	totalhours = pilotProject.getTotalhours();
    	} catch  (Exception ex) {
            throw new webschedulePresentationException("Error getting project information", ex);
        }
...
// check to see if the cancellation is less than 48 hours charge the pilot hours one hour.
// June 2007 check to see if the cancellation is less than 29 days charge the pilot hours one hour.
long numofhours = difference/3600000;
//if (difference < 172800000)
if (numofhours < 696)
	{
	...
	// need to check if some else cancelled the same time slot
	//move the old one last flag to false?
...
protected void saveC_event(String s_eventID, C_event theC_event)
    ...
    try {
    	    ...
    	    String[] message = {"This message is to confirm that you deleted the following event "+
				...
				"Please delete any other associated resources such as parking and the exam room",
				"Thank you",
    	    			" "}  ;
    	...
    	int todaymonth = cancelinfo.get(cancelinfo.MONTH);
    	int todayyear = cancelinfo.get(cancelinfo.YEAR);
	int todayh = cancelinfo.get(cancelinfo.HOUR);
	int todaymin = cancelinfo.get(cancelinfo.MINUTE);
    	Date cancelinfodate =  cancelinfo.getTime();
//System.out.println("Cancellation Date off cancel module "+cancelinfodate);
    	long cancelinfoms = cancelinfodate.getTime();
    	...
    	eventinfo.set(eventyear, eventmonth, eventday, starth, startm);
    	Date eventinfodate =   eventinfo.getTime();
//System.out.println("Event date off cancel module "+eventinfodate);	
    	long eventinfoms = eventinfodate.getTime();
	long difference = eventinfoms - cancelinfoms;
	long numofhours = difference/3600000;
	...
	if (numofhours < 0)
		...
		theC_event.setTodayh(todayh);
		theC_event.setTodaymin(todaymin);
