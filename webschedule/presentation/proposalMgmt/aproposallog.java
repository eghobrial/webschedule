...
* @since: Jan 2008
*@description: This shows the proposal log view for the committee memebers, through you can approve, send comments. 
*
*--------------------------------------------------------------*/
 ...
 *
 */
public class aproposallog extends BasePO
{
  ...
  public String buildcomhref (String proj_id)
    {
       String temps = "approveproposal.po?event=showapprove&proj_id="+proj_id;
       return temps;
     }
     
public String buildvcomhref (String proj_id)
    {
       String temps = "showrescomments.po?event=showcomresp&proj_id="+proj_id;
       return temps;
     }

 public String buildinternalcomhref (String proj_id)
    {
       String temps = "internalcomments.po?event=internal&proj_id="+proj_id;
       return temps;
     }

  public String buildapphref (String proj_id)
    {
       String temps = "showapproveconf.po?event=approve&proj_id="+proj_id;
       return temps;
     }
    ...
    {
        aproposallogHTML page = new aproposallogHTML();
	int today ,month ,year;
String	aproj_name , proj_id, comments,hreftxt,hreftxtc,hreftxtapp, hreftxtic,bookmark;
	int	status ;
	String cdate;
String pifname, pilname, piname;
String statuscode[] = {"Editable","Submitted","Internal Comments","Committe Comments","PI Responded","Approved","Checked","Accounts Added"};
   ...
   //     templateOption.removeAttribute("id");
	 ...
	 HTMLElement committeeCellTemplate = page.getElementCommittee();
	 ...
	 //   HTMLElement commentsCellTemplate = page.getElementComments();
		 ...
		 committeeCellTemplate.removeAttribute("id");	
...
pifname = theProposal.getUserFirstName();
	...
	{  
	  ...
	  page.setTextCommittee(bookmark);
...
System.out.println(" **** Current proposal  aproj_name off proposalstatus ****"+ aproj_name);
	...
	System.out.println(" **** User Lastname off aproposallog for approve ****"+ lastname);
	if (lastname.equals("Dileo")) {
	     hreftxtc = buildcomhref(proj_id);
	 } else {
	     hreftxtc = buildvcomhref(proj_id);
		}
			 System.out.println(" **** Current proposal  hreftxt off proposalstatus for approve ****"+ hreftxtc);
		String 	sendcomments = "Send Comments";	
			Element link1 = page.getElementById("Commentid");
			 HTMLAnchorElement mylink1 = (HTMLAnchorElement) link1;
	mylink1.setHref(hreftxtc);
	Text t1 = XMLCUtil.getFirstText(link1);
	t1.setData(sendcomments);
	 ...
	 hreftxtapp = buildapphref(proj_id);
	
	
	 Element link2 = page.getElementById("Approve");
	 HTMLAnchorElement mylink2 = (HTMLAnchorElement) link2;
	 mylink2.setHref(hreftxtapp);
	 Text t2 = XMLCUtil.getFirstText(link2);
	 t2.setData(approve);
	
	hreftxtic = buildinternalcomhref(proj_id);
			 System.out.println(" **** Current proposal  hreftxt off proposalstatus for internal ****"+ hreftxtic);
		String 	intcomments = "Internal Comments";	
			Element link3 = page.getElementById("internalonly");
			 HTMLAnchorElement mylink3 = (HTMLAnchorElement) link3;
	mylink3.setHref(hreftxtic);
	Text t3 = XMLCUtil.getFirstText(link3);
	t3.setData(intcomments);
