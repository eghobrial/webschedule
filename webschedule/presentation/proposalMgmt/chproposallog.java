 ...
 *
 */
public class chproposallog extends BasePO
{
  private static String PROPOSAL_ID = "proposalID";
    ...
    System.out.println(" **** Hi after set href off check proposal log my book is****"+bookmark);
  ...
  public String buildchkhref (String proj_id)
    {
       String temps = "checkroute.po?event=showroute&proj_id="+proj_id;
       return temps;
     }

/*  public String buildacchref (String proj_id)
    {
       String temps = "quickview.po?event=quickview&proj_id="+proj_id;
       return temps;
     }
*/
    ...
    {
        chproposallogHTML page = new chproposallogHTML();
...
String statuscode[] = {"Editable","Submitted","Internal Comments","Committe Comments","PI Responded","Approved","Checked","Accounts Added"};
   ...
   /*    	 try {
	...
	System.out.println(" **** Last name for check and route ****"+lastname);
	
	if (notauth==0){
	
	//if (lastname != "Dileo") {
            this.getSessionData().setUserMessage( "  You are not auth to check/route proposals ");
...
System.out.println(" **** Current proposal  aproj_name off proposalstatus ****"+ aproj_name);
	...
	// System.out.println(" **** Current proposal  comments off proposalstatus ****"+ comments);		
		...
		hreftxtchk = buildchkhref(proj_id);
	//	hreftxtacc = buildacchref(proj_id);
		
		
			Element link2 = page.getElementById("Check");
			 HTMLAnchorElement mylink2 = (HTMLAnchorElement) link2;
	mylink2.setHref(hreftxtchk);
	Text t2 = XMLCUtil.getFirstText(link2);
	t2.setData(check);
	
	
	/*		Element link3 = page.getElementById("Route");
