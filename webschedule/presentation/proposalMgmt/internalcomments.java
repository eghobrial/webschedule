 ...
 *
 */
public class internalcomments extends BasePO
{
 private static String NOTES = "notes";
  private static String PROJ_ID = "proj_id"; 
    ...
    {
        internalcommentsHTML page = new internalcommentsHTML();
 ...
 if(null != this.getComms().request.getParameter(NOTES)) {
     ...
     *
     */
    public String showInternalPage(String errorMsg)
     throws HttpPresentationException, webschedulePresentationException
    {
        internalcommentsHTML page = new internalcommentsHTML();
	 String notes = this.getComms().request.getParameter(NOTES);
 ...
 try {
	  ...
	  comments = theProposal.getIntcomments();
...
if(null != notes && notes.length() != 0) {
	...
	System.out.println("Handle Internal visited  ");
	System.out.println("proposal id  =   "+proj_id);
...
//	  this.setProposalID(proj_id);
   ...
   //	theProposal.setStatus(theProposal.getStatus());
	theProposal.setStatus(2);
	theProposal.setIntcomments(notes);
   ...
   // theProposal.setAnimTransDate(Date.valueOf(adts));
    ...
    System.out.println(" trying to saving a proposal three ");
     ...
     /* protected void sendemail (Proposal theProposal)
        throws HttpPresentationException, webschedulePresentationException
    {  
 String pifname, pilname, to, proposalid, proj_name, aproj_name,subject,from,intcomments;
String [] message;
 int	status ;
...
String statuscode[] = {"Editable","Submitted","Internal Comments","Committe Comments","PI Responded","Approved","Checked","Accounts Added"};
 ...
 // to = "7tcommittee";   
     ...
     intcomments = theProposal.getComments();
     status = theProposal.getStatus();
      } catch(Exception ex) {
        throw new webschedulePresentationException("Cannot retrieve proposal info", ex);    
        }
	
subject = "Proposal "+aproj_name+" Internal Only Committee Comments ";
String[] msgText1 = {
                      "Proposal ID number: "+proposalid+"\n",
		      "PI Name "+pilname+",",
		      "Has the following comments (for committee internal Use Only)\n",
		      "  "+ intcomments+"\n",
		    " "};	
