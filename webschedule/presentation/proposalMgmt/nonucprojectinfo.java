 ...
 *
 */
public class nonucprojectinfo extends BasePO
{
 ...
 public String handleProjectinfo()
	...
	try
	{
            ...
            this.getSessionData().setUserMessage(proj_id + "  Please choose a valid project ");
                 throw new ClientPageRedirectException(NONUCSDPROJECTS_PAGE);
                 // Show error message that project not found
	} else{
		 
	  this.setProjectID(proj_id);
            throw new ClientPageRedirectException(NONUCPROJECTINFO_PAGE);
	     	}   
	    } catch (Exception ex) {
...
//	String  oneop = this.getComms().request.getParameter(ONEOP);
   ...
   //   	String  fpemail ;
	...
	String edst, emst, eyst, expst;
        ...
        nonucprojectinfoHTML page = new nonucprojectinfoHTML();
	...
	//    indexnum = theProject.getIndexnum();
	   aanum = theProject.getAccountid();
	   
	    Baline1 = theProject.getBilladdr1();
	    //temp placement for po
	    ponum = theProject.getBilladdr2();
...
/*System.out.println(" Contact info off Preview "+ cname+cphone+cemail);
	...
	page.setTextCemail(cemail);*/
	  ...
	  if (ponum == null) 
	    page.setTextPonum("Not on File");
	    else  page.setTextPonum(ponum);
	    
	  if (aanum == null) 
	    page.setTextAanum("Not on File");
	    else  page.setTextAanum(aanum);
